...
export function getBlocksBatches(fromBlock: number, toBlock: number, numOfBatches: number): Array<[number, number]> {
  const batchSize  = Math.ceil((toBlock - fromBlock) / numOfBatches)
